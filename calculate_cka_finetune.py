...
if __name__ == '__main__':
    ...
    csv_files = [f for f in os.listdir(f"outputs/{input_file}/similarity_matrix_finetune") if f.endswith('.csv')]
    ...
    for i in range(num_files):
        for j in range(num_files):
            if i <= j:  # 上三角行列のみ計算
                X = pd.read_csv(os.path.join(f'outputs/{input_file}/similarity_matrix_finetune', csv_files[i]), header=None).values[1:, 1:]
                Y = pd.read_csv(os.path.join(f'outputs/{input_file}/similarity_matrix_finetune', csv_files[j]), header=None).values[1:, 1:]
    ...
    similarity_df.to_csv(f"outputs/{input_file}/cka_matrix/cka_matrix_finetune.csv")
