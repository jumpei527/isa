...
output_dir = f'outputs/{input_file}/similarity_matrix'
...
for model_dir in model_directories:
    ...
    # Pandas DataFrameに変換
    similarity_df = pd.DataFrame(similarity_matrix_np, index=[f"Q{i+1}" for i in range(len(input_texts))],
                                 columns=[f"Q{j+1}" for j in range(len(input_texts))])

    # 類似度行列をCSVとして保存
    similarity_df.to_csv(f'{output_dir}/{model_dir}.csv', index=True)
